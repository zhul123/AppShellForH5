...
public class StatusBarUtil {
    ...
    // 设置为白字
    public static void setWhiteTextStatusBar(Activity activity) {
     ...
     * @param light    是否高亮 true是 false 否
     */
    ...
    public static void setTransparencyBar(Activity activity) {
        ...
        Window window = activity.getWindow();
        if ((StatusBarUtil.isMeizu() || StatusBarUtil.isXiaomi())) {
            //--不支持蒙层效果
            window.setFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS,
                    WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            return;
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && supportTransclentStatusBar6()) {
                ...
                // 采取setStatusBarColor的方式，部分机型不支持，那就纯黑了，保证状态栏图标可见,部分机型支持Color.TRANSPARENT
    ...
    private static boolean supportTransclentStatusBar6() {
        //联想
        final String board01 = Build.MODEL;
        boolean isZUKZI = board01 != null && board01.toLowerCase().contains("zuk z1");
        final String board02 = Build.MODEL;
        //中兴
        boolean isZTKC2016 = board02 != null && board02.toLowerCase().contains("zte c2016");
        return !(isZUKZI || isZTKC2016);
    }
    ...
    public static void setStatusBarViewHeight(View mStatusBarView) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            //4.4及其以下
            return;
        }
        ...
        int statusBarHeight = StatusBarUtil.statusBarHeight == 0 ? default_height : StatusBarUtil.statusBarHeight;
        if (mStatusBarView.getHeight() != statusBarHeight) {
            try {
